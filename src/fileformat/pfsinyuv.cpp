...
bool parseFileName(const char * fileNameIn, int * width,  int * height, int * bitdepth, pfs::ColorSpace * colorspace, ChromaFormat * chromaFormat, int * fps){
  try{
    ...
    while ((pos = fileName.find(delimiter)) != std::string::npos || (pos = fileName.find('.')) != std::string::npos) {
...
void readFrames( int argc, char* argv[] )
{
  ...
  if(filename != NULL){
    if( !strcmp(filename, "-") ) {
      // Use stdin instead of file for input
      VERBOSE_STR << "reading from stdin" << std::endl;
      fh = stdin;
    } else {
  ...
  while( (currentFrame <= frame_max && frame_stride > 0)
    ...
    if( fh != stdin && reader.seekToFrame(currentFrame) ) {
