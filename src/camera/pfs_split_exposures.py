...
parser.add_argument('--maxexp', '-x', dest='maxexp', default=12, 
                    help='The maximum number of exposures in each exposure stack. Only up to the number of maximum exposures will be put into a single stack.')
...
for filename in sorted(os.listdir(args.dir)):
    if filename[-4:] == '.ARW' or filename[-4:] == '.CR2':
        ...
        if capture_time!=None and prev_capture_time!=None:
            ...
            if capt_time_diff > args.threshold or len(exp_stack) > int(args.maxexp):
                print( '===============')
                if len(exp_stack) < int(args.minexp):
